           ...
           /*
            for(tempi=0;tempi<288;tempi++)
            {
                if(SDODATA[tempi*4+3] == 4)
                {
                    ...
                    ec_SDOwrite(1,SDODATA[tempi*4],SDODATA[tempi*4+1],FALSE,os,&ob1,EC_TIMEOUTRXM);
                }
                else if(SDODATA[tempi*4+3] == 2)
                {
                    ...
                    ec_SDOwrite(1,SDODATA[tempi*4],SDODATA[tempi*4+1],FALSE,os,&ob2,EC_TIMEOUTRXM);
                }
                else if(SDODATA[tempi*4+3] == 1)
                {
                    os=sizeof(ob3); ob3 = SDODATA[tempi*4+2];
                    ec_SDOwrite(1,SDODATA[tempi*4],SDODATA[tempi*4+1],FALSE,os,&ob3,EC_TIMEOUTRXM);
                }
                else
                {
                    //nothing
...
void MainWindow:: UpStatus()
{
    ...
    if(EcatError)
    {
        str.sprintf("Errorjq: %s:",ec_elist2string());
        ui->infoBrowser->append(str);
        qDebug() << EcatError;
    ...
    {
        str.sprintf("no error",Count);
        ui->Errcodelabel->setText(str);
    }
    if(SendPdo && ec_slave[1].state == EC_STATE_OPERATIONAL)
    {
        ...
        *(ec_slave[1].outputs + 22 ) = (OutputData.NegTorLim & 0xff00) >> 8;
        ...
        InputData.StatusWord = *(ec_slave[0].inputs+1);
        InputData.StatusWord <<= 8;
        InputData.StatusWord |= *(ec_slave[0].inputs);
        InputData.LenzeStatus = *(ec_slave[0].inputs+3);
        InputData.LenzeStatus <<= 8;
        InputData.LenzeStatus |= *(ec_slave[0].inputs+2);
        InputData.ModeStatus = *(ec_slave[0].inputs + 4);
        InputData.ErrorCode = *(ec_slave[0].inputs + 6);
        InputData.ErrorCode <<= 8;
        InputData.ErrorCode |= *(ec_slave[0].inputs + 5);
        InputData.VelValue |= *(ec_slave[0].inputs+10);
        InputData.VelValue <<= 8;
        InputData.VelValue |= *(ec_slave[0].inputs+9);
        InputData.VelValue <<= 8;
        InputData.VelValue |= *(ec_slave[0].inputs+8);
        InputData.VelValue <<= 8;
        InputData.VelValue |= *(ec_slave[0].inputs+7);
        InputData.TorValue = 0;
        InputData.TorValue |= *(ec_slave[0].inputs+12);
        InputData.TorValue <<= 8;
        InputData.TorValue |= *(ec_slave[0].inputs+11);
        InputData.PosiValue |= *(ec_slave[0].inputs+16);
        InputData.PosiValue <<= 8;
        InputData.PosiValue |= *(ec_slave[0].inputs+15);
        InputData.PosiValue <<= 8;
        InputData.PosiValue |= *(ec_slave[0].inputs+14);
        InputData.PosiValue <<= 8;
        InputData.PosiValue |= *(ec_slave[0].inputs+13);
...
void MainWindow::on_SDO_read_clicked()
{
    unsigned int SDODATA[1152]={
    4337,2,0x14000000,4,
    9472,0,0x0000,2,
