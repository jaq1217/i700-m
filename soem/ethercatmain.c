...
/**Transmit processdata to slaves for motor    by jaq
  *there are two datagrams will be Transmit
  * first datagram:use cmd "LRW" for sending motion data and receiving status data
  *second datagram:use cmd "BRD" for requseting  AL Status regiter data
  * the version only use single motor ,only one slaver
  */
int ecx_sendmotor_processdata_group(ecx_contextt *context, uint8 group)
{
    uint32 LogAdr;
    uint16 w1, w2;
    ...
    length = context->grouplist[group].Ibytes;//45
    ...
    if (length)
    {
        ...
        w1 = LO_WORD(LogAdr);
        w2 = HI_WORD(LogAdr);
        ecx_setupdatagram(context->port, &(context->port->txbuf[idx]), EC_CMD_LRW, idx, w1, w2, length, data);
        if(first)
        {
        ...
        ecx_pushindex(context, idx, data, length);
...
int ec_sendmotor_processdata_group(uint8 group)
{
   return ecx_sendmotor_processdata_group (&ecx_context, group);
}
...
int ec_sendmotor_processdata(void)
{
   return ec_sendmotor_processdata_group(0);
}
