...
int ecx_TxPDO(ecx_contextt *context, uint16 slave, uint16 TxPDOnumber , int *psize, void *p, int timeout)
{
   ...
   //SDOp->Index = 0x1a00;//jaq
   //SDOp->SubIndex = 0x01;//jaq
...
int ecx_readPDOassign(ecx_contextt *context, uint16 Slave, uint16 PDOassign)
{
   ...
   printf(" wkc:%d,rdl:%d,rdat:%d,PDOassign:%d\n",wkc,rdl,rdat,PDOassign);
      ...
      for (idxloop = 1; idxloop <= nidx; idxloop++)
      {
         ...
         if (idx > 0)
         {
            ...
            for (subidxloop = 1; subidxloop <= subidx; subidxloop++)
            {
               ...
               printf(" idx:%x,subidxloop:%x,rdl:%x,rdat2:%x\n",idx,subidxloop,rdl,rdat2);
               rdat2 = etohl(rdat2);
               printf(" rdat2:%x\n",rdat2);
   ...
   if ((wkc > 0) && (context->PDOassign->n > 0))
   {
      ...
      for (idxloop = 1; idxloop <= nidx; idxloop++)
      {
         ...
         if (idx > 0)
         {
            ...
            printf("nidx,context->PDOassign->n:%x\n",nidx);
            printf("idx,context->PDOassign->index[%d - 1]:%x\n",idxloop,idx);
            printf("subidx,context->PDOdesc->n:%x\n",subidx);
            /* extract all bitlengths of SDO's */
            for (subidxloop = 1; subidxloop <= subidx; subidxloop++)
            {
               bsize += LO_BYTE(etohl(context->PDOdesc->PDO[subidxloop -1]));
                printf("subidxloop,context->PDOdesc->PDO[%d]:%x\n",subidxloop,context->PDOdesc->PDO[subidxloop -1]);
            }
            printf("\n\n",subidxloop,subidx);
         }
...
int ecx_readPDOmapCA(ecx_contextt *context, uint16 Slave, int *Osize, int *Isize)
{
   ...
   *Osize = 0;
   ...
   if ((wkc > 0) && (context->SMcommtype->n > 2))
   {
      ...
      freopen("readPDOmapCA.txt","w",stdout);
      printf("readPDOassignCA\n");
      /* iterate for every SM type defined */
      for (iSM = 2 ; iSM <= nSM ; iSM++)
      {
